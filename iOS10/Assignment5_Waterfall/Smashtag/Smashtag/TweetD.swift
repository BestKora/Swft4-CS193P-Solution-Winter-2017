...
class TweetD: NSManagedObject {
    class func findOrCreateTweet(matching twitterInfo: Tweet,
                                 in context: NSManagedObjectContext) throws -> TweetD
    {
    ...
    class func findTweetAndCheckMentions(for twitterInfo: Tweet,
                                    ...
                                    in context: NSManagedObjectContext) throws -> TweetD
    {
        do {
            ...
            for hashtag in hashtags{
                _ = try? MentionD.checkMention(for: tweet,
    ...
    class func newTweets( for twitterInfos: [Tweet],
                          with searchTerm: String,
        ...
        do {
            ...
            for unique in newsSet{
                if let index = twitterInfos.index(where: {$0.identifier == unique}){
                    _ = try? TweetD.findTweetAndCheckMentions(for: twitterInfos[index],
                                                             with:searchTerm,
                                                             in: context)
  ...
  //  ----- Remove Tweets ----
    ...
    override public func prepareForDeletion() {
        if let mentionsSet = mentions as? Set<MentionD>, mentionsSet.count > 0  {
