...
struct CalculatorBrain {
    ...
    private var operations : Dictionary <String,Operation> = [
        ...
        "±": Operation.unaryOperation({ -$0 },nil, nil, nil),
        "√": Operation.unaryOperation(sqrt,nil, { $0 < 0 ? "√ отриц. числа" : nil }, nil),
        
        "cos"   : Operation.unaryOperation(cos,nil, nil, {cos($0.degreesToRadians)}),
        "sin"   : Operation.unaryOperation(sin,nil, nil, {sin($0.degreesToRadians)}),
        "tan"   : Operation.unaryOperation(tan,nil, nil, {tan($0.degreesToRadians)}),
        "sin⁻¹" : Operation.unaryOperation(asin,nil,
                { $0 < -1.0 || $0 > 1.0 ? "не в диапазоне [-1,1]" : nil }, {asin ($0).radiansToDegrees}),
        "cos⁻¹" : Operation.unaryOperation(acos,nil,
                { $0 < -1.0 || $0 > 1.0 ? "не в диапазоне [-1,1]" : nil }, {acos ($0).radiansToDegrees}),
        "tan⁻¹" : Operation.unaryOperation(atan, nil, nil, {atan ($0).radiansToDegrees}),
        
        "ln"    : Operation.unaryOperation(log,nil,{ $0 <= 0 ? "ln отриц. числа" : nil }, nil),
        "x⁻¹"   : Operation.unaryOperation({1.0/$0},
                {"(" + $0 + ")⁻¹"},{ $0 == 0.0 ? "Деление на нуль" : nil }, nil),
        "х²"    : Operation.unaryOperation({$0 * $0}, { "(" + $0 + ")²"}, nil, nil),
        
        "×": Operation.binaryOperation(*, nil, nil, 1),
        "÷": Operation.binaryOperation(/, nil, { $1 == 0.0 ? "Деление на нуль" : nil }, 1),
        ...
        (result: Double?, isPending: Bool, description: String, error: String?){
            ...
            func performOperation(_ symbol: String) {
                if let operation = operations[symbol]{
                    ...
                    case .unaryOperation (let function, var descriptionFunction, let validator, let functionDegrees):
                        guard cache.accumulator != nil else {return}
                            error = validator?(cache.accumulator!)
                            if functionDegrees != nil && !isRadian {
                                cache.accumulator = functionDegrees! (cache.accumulator!)
                                descriptionFunction = {symbol + "°" + "(" + $0 + ")"}   //standard degrees
                            } else {
