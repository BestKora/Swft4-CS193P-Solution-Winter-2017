...
//        }
    ...
    public func fetch(_ handler: @escaping (PropertyList?) -> Void) {
        performTwitterRequest(handler: handler)
    }
    ...
    func performTwitterRequest(handler: @escaping (PropertyList?) -> Void) {
        ...
        if let twitterAccountId = Twitter.sharedInstance().sessionStore.session()?.userID {
            ...
            let request = client.urlRequest(withMethod: "GET", url: url, parameters: parameters, error: &clientError)

            client.sendTwitterRequest(request) { (response, jsonResponse, error) -> Void in
